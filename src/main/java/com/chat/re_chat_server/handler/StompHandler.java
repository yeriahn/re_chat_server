package com.chat.re_chat_server.handler;
...
public class StompHandler implements ChannelInterceptor {
    ...
    public Message<?> preSend(Message<?> message, MessageChannel channel) {
        ...
        if(StompCommand.CONNECT == accessor.getCommand()) {
            ...
            log.info("StompCommand,SUBSCRIBE accessor : {}" , accessor);
            // head정보에서 구독 destination 정보를 얻고, roomId를 추출한다.
            String roomId = chatService.getRoomId(Optional.ofNullable((String) message.getHeaders().get("simpDestination")).orElse("InvalidRoomId"));
            //채팅방에 들어온 클라이언트 sessionId를 roomId와 맵핑해놓는다.(나중에 특정 세션이 어떤 채팅방에 들어가 있는지 알기 위함)
            ...
            log.info("StompCommand.DISCONNECT accessor : {}", accessor);
